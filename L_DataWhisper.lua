module ("L_DataWhisper", package.seeall)
...
--local DataWhisper = interface {
  debugOn                 = false;
  CACHE_HEADERS           = true;     -- improves file access performance for read and write
...
struct = {version = '2014.02.13  @akbooer'}
...
function struct.pack (fmt, ...)
  local fmt = formatCache[fmt] or checkFormat (fmt)  
  local result = {}
  local args = {...}
  local f, v
...
function WhisperException(Exception) 
...
local function aggregate (aggregationMethod, knownValues)
  ...
  local function err (Xs) WhisperException ("Unrecognized aggregation method %s", aggregationMethod) end
...
local function header (metadata)
  ...
  --  # return nothing
    ...
    if fromTime  < oldestTime then fromTime = oldestTime end            --  # Range requested is partially beyond retention, adjust
...
function __file_open (path, mode, fct, silent)
