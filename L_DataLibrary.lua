...
local function parser (example)
  ...
  local version = "2013.12.28  @akbooer"

  local other  = true   -- set to true to suppress error for unknown parameters (which end up in "other" category)
  ...
  local function allowOther (flag)
    other = flag or true
  end
  ...
  local function help ()
    ...
    for class, params in pairs (syntax) do
      ...
      for _,b in pairs (params) do
        local val = b.vals
        if b.help then                                -- parameters without help text remain 'undocumented'
  ...
  local function parse (lul_parameters)
    ...
    for param, value in pairs(lul_parameters) do
      ...
      if fullName then 
        ...
        if type(cmd.vals) == "string" then
          if cmd.vals == "number" then finalValue = tonumber (value)  -- convert numeric value 
          elseif cmd.vals == "string" then finalValue = value         -- string type
          else                                                        -- assume string library pattern (with captures)
            finalValue = {string.match(value, cmd.vals)}              -- return possibly multiple captures in a list
            if #finalValue == 0 then finalValue = nil end             -- no match found
          end
        end
      ...
      elseif other then                 -- collect 'other' pararameters, or...
